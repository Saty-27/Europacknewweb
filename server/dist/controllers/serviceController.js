...
const getServicePages = async (req, res) => {
    try {
        ...
        const pages = await ServicePage_1.default.find(query).sort({ createdAt: -1 }).lean();
...
const getServiceBySlug = async (req, res) => {
    try {
        ...
        const page = await ServicePage_1.default.findOne({ slug, visible: true, status: 'published' }).lean();
        if (!page)
