...
const getIndustries = async (req, res) => {
    try {
        ...
        const industries = await Industry_1.default.find(filter).sort({ order: 1, createdAt: -1 }).lean();
...
const getIndustryBySlug = async (req, res) => {
    try {
        const industry = await Industry_1.default.findOne({ slug: req.params.slug, visible: true }).lean();
        if (!industry)
